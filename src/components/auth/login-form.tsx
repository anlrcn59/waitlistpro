...
function mapError(message: string, isEn: boolean): string {
  ...
  if (message.includes("Email not confirmed"))
    return isEn
      ...
      : "Email adresini henüz onaylamamışsın. Gelen kutunu kontrol et.";
...
const URL_ERRORS: Record<string, { tr: string; en: string }> = {
...
export function LoginForm({ urlError, dict }: Props) {
  ...
  // Fallback Turkish strings when dict not provided (legacy /login route)
  const d = dict ?? {
    title: "Giriş Yap",
  ...
  const [error, setError] = useState(() => {
    if (!urlError) return "";
    const e = URL_ERRORS[urlError];
    return e ? (isEn ? e.en : e.tr) : (isEn ? "An error occurred." : "Bir hata oluştu.");
  });
      ...
      <div className="rounded-xl border bg-card p-6 shadow-xs space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="login-email">{d.email}</Label>
            <Input
              id="login-email"
              type="email"
              value={email}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="login-password">{d.password}</Label>
            <Input
              id="login-password"
              type="password"
              value={password}
